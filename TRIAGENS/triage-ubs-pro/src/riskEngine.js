// Motor simples baseado em regras (educacional).
...
function possibleConditions(flags) {
  const suggestions = [];
...
export function engineTriagem(input) {
  const {
    nome = '', idade = '', genero = '', alergias = '', sintomas = [], descricaoLivre = '', gestante = false
  } = input || {};
  ...
  let score = 0;
  const flags = new Set();
  const text = [desc, ...symptomList].join(' | ');
  keywords.forEach(({k, score: s, tag}) => {
    for (const kw of k) {
      if (text.includes(kw)) { score += s; flags.add(tag); break; }
    }
  });
  ...
  return {
    ...
    resultado: {
      ...
      hipotesePrincipal: conditions[0] || 'Sem hipótese aparente'
    }
  };
}